...
class PartymeisterCoreApiGuestTest extends TestCase
{
    ...
    public function can_show_a_single_guest()
    {
        ...
        $this->json('GET',
            '/api/guests/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(200)->seeJson([
            'name' => $record->name
        ]);
    }
    ...
    public function fails_to_show_a_single_guest_without_permission()
    {
        $record = create_test_guest();
        $this->json('GET',
            '/api/guests/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(403)->seeJson([
            'error' => 'Access denied.'
        ]);
    }
    ...
    public function can_search_for_a_guest()
    {
        ...
        $records = create_test_guest(10);
        $this->json('GET',
            '/api/guests?api_token=' . $this->user->api_token . '&search=' . $records[2]->name)->seeStatusCode(200)->seeJson([
            'name' => $records[2]->name
        ]);
    }
    ...
    public function can_show_a_second_guest_results_page()
    {
        ...
        $this->json('GET',
            '/api/guests?api_token=' . $this->user->api_token . '&page=2')->seeStatusCode(200)->seeJson([
            'current_page' => 2
        ]);
    }
    ...
    public function fails_if_trying_to_modify_a_guest_without_payload()
    {
        ...
        $record = create_test_guest();
        $this->json('PATCH',
            '/api/guests/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(422)->seeJson([
            'name' => [ 'The name field is required.' ]
        ]);
    }
