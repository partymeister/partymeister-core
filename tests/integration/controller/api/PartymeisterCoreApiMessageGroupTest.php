...
class PartymeisterCoreApiMessageGroupTest extends TestCase
{
    ...
    public function can_show_a_single_message_group()
    {
        ...
        $this->json('GET',
            '/api/message_groups/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(200)->seeJson([
            'name' => $record->name
        ]);
    }
    ...
    public function fails_to_show_a_single_message_group_without_permission()
    {
        $record = create_test_message_group();
        $this->json('GET',
            '/api/message_groups/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(403)->seeJson([
            'error' => 'Access denied.'
        ]);
    }
    ...
    public function can_search_for_a_message_group()
    {
        ...
        $records = create_test_message_group(10);
        $this->json('GET',
            '/api/message_groups?api_token=' . $this->user->api_token . '&search=' . $records[2]->name)->seeStatusCode(200)->seeJson([
            'name' => $records[2]->name
        ]);
    }
    ...
    public function can_show_a_second_message_group_results_page()
    {
        ...
        $this->json('GET',
            '/api/message_groups?api_token=' . $this->user->api_token . '&page=2')->seeStatusCode(200)->seeJson([
            'current_page' => 2
        ]);
    }
    ...
    public function fails_if_trying_to_modify_a_message_group_without_payload()
    {
        ...
        $record = create_test_message_group();
        $this->json('PATCH',
            '/api/message_groups/' . $record->id . '?api_token=' . $this->user->api_token)->seeStatusCode(422)->seeJson([
            'name' => [ 'The name field is required.' ]
        ]);
    }
    ...
    public function fails_if_trying_to_delete_a_non_existing_message_group()
    {
        ...
        $this->json('DELETE', '/api/message_groups/1?api_token=' . $this->user->api_token)->seeStatusCode(404)->seeJson([
            'message' => 'Record not found'
        ]);
    }
