...
class CallbackForm extends Form
{
    public function buildForm()
    {
        $this
            ...
            ->add('action', 'select', ['label' => trans('partymeister-core::backend/callbacks.action'), 'choices' => (trans('partymeister-core::backend/callbacks.actions'))])
            ->add('payload', 'static', ['label' => trans('partymeister-core::backend/callbacks.payload')])
            ->add('title', 'text', ['label' => trans('partymeister-core::backend/callbacks.title'), 'rules' => 'required'])
            ...
            ->add('destination', 'select', ['label' => trans('partymeister-core::backend/callbacks.destination'), 'choices' => (trans('partymeister-core::backend/callbacks.destinations'))])
            ->add('has_fired', 'checkbox', ['label' => trans('partymeister-core::backend/callbacks.has_fired')])
            ->add('submit', 'submit', ['attr' => ['class' => 'btn btn-primary competition-submit'], 'label' => trans('partymeister-core::backend/callbacks.save')]);
    }
}
