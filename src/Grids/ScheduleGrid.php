...
class ScheduleGrid extends Grid
{
    protected function setup()
    {
        ...
        $this->addAction(
            trans('partymeister-core::backend/schedules.generate_slides'),
            'backend.schedules.slides.index',
            [ 'class' => 'btn-primary' ]
        )
             ->needsPermissionTo('schedules.read')
             ->onCondition('event_count', 0, '>');
        $this->addAction(
            trans('motor-backend::backend/global.show'),
            'backend.schedules.show',
            [ 'class' => 'btn-primary' ]
        )->needsPermissionTo('schedules.read')->onCondition('event_count', 0, '>');
